...
export default class MenuSettings extends React.Component {
  ...
  getMenuItems() {
    ...
    function itemToMarkup({ label, action, option, type, required }, index) {
      ...
      if (type === 'checkbox') {
        ...
        return (
          <cq-item stxsetget={action} key={label} >
            {label}
            <span className="ciq-checkbox ciq-active"><span /></span>
          </cq-item>
        );
      }
      if (type === 'radio') {
        return (
          <cq-item key={label}>
            {option && <span className="ciq-edit" stxtap={option} />}
            <div stxsetget={action}>{label}<span className="ciq-radio"><span /></span></div>
          </cq-item>
        );
      }
      if (type === 'timezone') {
      ...
      if (type === 'languages') {
        return (
          <cq-item stxsetget="Layout.Language()" key={type}><cq-flag /><cq-language-name>Change Language</cq-language-name></cq-item>
        );
      }
      if (type === 'themes') {
        return (
          <cq-themes ref={self.themesRef} key={type}>
            ...
            <cq-item stxtap="newTheme()"><cq-plus />{label}</cq-item>
          </cq-themes>
        );
      }
    }
