...
const { SYMLOOKUP_URI } = globalAppConfig.getProperties();
...
export default class LookupDriver {
  ...
  initPreloaded() {
    ...
    preloaded.forEach(symbol => {
      ...
      symbol.addEventListener('click', () => this.chartEngine.addSeries(ticker, { isComparison: true, color: colors[ticker], symbolObject: { symbol: ticker, name: ticker } }));
    });
  }
}
